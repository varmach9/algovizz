...
const Editdis = () => {
    ...
    const lcs=async()=>{
        ...
        setrun(0)
        document.getElementById("res").innerHTML=mat[b.length][a.length]
    }

  return (
    <div style={{marginLeft:"50px"}}>
    <div style={{display:"flex",justifyContent:"center",width:"1300px",margin:"10px"}}><h1 style={{fontWeight:"bolder",color:"Red", textShadow: "0 0 1px blue",paddingLeft:"235px",paddingRight:"5px"}}>Edit Distance</h1></div>
    ...
    <form onSubmit={(e)=>{
        ...
        <h6 style={{marginTop:"10px"}}>Adjust Speed:</h6>
            ...
            <button class="btn btn-warning btn-sm" onClick={lcs} style={{marginTop:"0px",marginBottom:"10px",marginLeft:"0px"}}>Visualize DP</button>  
        </div>
        <div style={{display:"flex"}}>
        ...
        <div id="res" style={{marginLeft:"10px",backgroundColor:"yellow"}}></div>
        </div>
        <div style={{marginLeft:"0px",marginRight:"10px",marginTop:"10px"}}>
        <div>
  <p>
    The Edit Distance problem aims to find the minimum number of operations required to transform one string into another. 
    Here's how the DP approach solves the problem:
  </p>
  <ol>
    <li>
      Create a table to store the minimum edit distance for each pair of prefixes of the two strings.
    </li>
    <li>
      Initialize the first row and column of the table with incremental values, as transforming any prefix to an empty string requires deleting all characters.
    </li>
    <li>
      Iterate through the remaining cells of the table in a top-down manner.
      <ul>
        <li>
          If the current characters of the two strings match, the minimum edit distance remains the same as the diagonal cell (i-1, j-1).
        </li>
        <li>
          Otherwise, take the minimum edit distance from the cell above (i-1, j), the cell to the left (i, j-1), and the diagonal cell (i-1, j-1), and increment it by 1 to account for the current operation.
        </li>
      </ul>
    </li>
    <li>
      The bottom-right cell of the table will contain the minimum edit distance for the entire strings.
    </li>
    <li>
      To obtain the sequence of operations, trace back the table from the bottom-right cell following the minimum edit distance calculation rules.
    </li>
  </ol>
</div>
